package  com.gt.playnow.data.ui.indefinitepagerindicator
...
class IndefinitePagerIndicator @JvmOverloads constructor(
    ...
    @ColorInt
    private var dotColor: Int = ContextCompat.getColor(this.context, R.color.dotIindicatorUnSelected)
    @ColorInt
    ...
    public fun setDotCount(dotCount: Int) {
    ...
    internal inner class InternalRecyclerScrollListener : RecyclerView.OnScrollListener() {
        ...
        private fun setIntermediateSelectedItemPosition(mostVisibleChild: View?) {
            with(mostVisibleChild?.let { recyclerView?.findContainingViewHolder(it)?.adapterPosition }!!) {
